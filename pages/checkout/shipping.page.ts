...
class ShippingPage extends BasePage {
  ...
  constructor(page: Page) {
    super(page);
    this.orderSummaryExpandButton = page.locator(
      "//div[contains(@class, 'items-in-cart')]"
    );
  ...
  async getProductName(productName: string) {
    return this.page
      .locator(
        `//strong[@class='product-item-name' and text()='${productName}']`
      )
      .first()
      .innerText();
  }
  ...
  async getQuantity(productName: string) {
    return this.page
      .locator(
        `//strong[text()='${productName}']/following-sibling::div[@class='details-qty']/span[@class='value']`
      )
      .first()
      .innerText();
  }
  ...
  async getPrice(productName: string) {
    return (
      await this.page
        .locator(
          `//strong[text()='${productName}']/ancestor::div[contains(@class, 'product-item')]/div[@class='subtotal']//span`
        )
        .first()
        .innerText()
    ).replace(/[^0-9.]/g, "");
  ...
  async getSize(productName: string) {
    return this.page
      .locator(
        `//strong[text()='${productName}']/ancestor::div[contains(@class, 'product-item')]//dl[@class='item-options']/dd[1]`
      )
      .first()
      .innerText();
  }
  ...
  async getColor(productName: string) {
    return this.page
      .locator(
        `//strong[text()='${productName}']/ancestor::div[contains(@class, 'product-item')]//dl[@class='item-options']/dd[2]`
      )
      .first()
      .innerText();
  }
  ...
  async fillStreetAddressTextBox(streetAddress: string) {
    await TestHelper.clickClearEnterTextBox(
      this.streetAddressTextBox,
      streetAddress
    );
  }
  ...
  async selectShippingMethod(shippingMethod: string) {
    const shippingMethodLocator = this.page.locator(
      `//td[contains(@class, 'col-carrier') and text()='${shippingMethod}']/parent::tr`
    );
  ...
  async enterShippingAddressAndProceedToPayment(
